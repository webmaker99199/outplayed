    ...
    fetch('/api/outplayed/products')
      ...
      .then(data => {
        ...
        function renderCarousel() {
          const videoFirst = Boolean(streamableId);
          const showingVideo = Boolean(streamableId && currentIndex === 0);
          const imageIndex = videoFirst ? currentIndex - 1 : currentIndex;
          const currentMedia = showingVideo
            ...
            : `<img src="${images[imageIndex]}" alt="${product.name || 'Product Image'} - Photo ${imageIndex + 1}" style="display: block; width: 100%; height: 100%; min-width: 0; min-height: 0; object-fit: contain; object-position: center; border-radius: 1rem;" />`;
          ...
          carouselWrap.innerHTML = `
            ...
            <div style="padding-top: 16px; display: flex; gap: 10px; overflow-x: auto;">
              ${streamableId ? `<button data-index="0" aria-label="View product video" style="position: relative; height: 64px; width: 80px; flex-shrink: 0; border-radius: 12px; overflow: hidden; border: 2px solid ${currentIndex === 0 ? '#a855f7' : 'rgba(255,255,255,0.2)'}; opacity: ${currentIndex === 0 ? '1' : '0.6'}; cursor: pointer; background: #171021; color: white; font-size: 12px; font-weight: 600;" class="thumbnail-btn">▶ Video</button>` : ''}
              ${images.map((img, idx) => `
                <button data-index="${videoFirst ? idx + 1 : idx}" aria-label="View image ${idx + 1}" style="position: relative; height: 64px; width: 80px; flex-shrink: 0; border-radius: 12px; overflow: hidden; border: 2px solid ${(videoFirst ? idx + 1 : idx) === currentIndex ? '#a855f7' : 'rgba(255,255,255,0.2)'}; opacity: ${(videoFirst ? idx + 1 : idx) === currentIndex ? '1' : '0.6'}; cursor: pointer; transition: all 0.2s;" class="thumbnail-btn">
